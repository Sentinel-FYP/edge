...
async def main():
    ...
    await camera.fetch_registered_cameras(api_client)
    await camera.connect_to_cameras(sio)
    ...
    process_cameras(
        camera.CONNECTED_CAMERAS.values(), sio, asyncio.get_event_loop(), api_client
    )
